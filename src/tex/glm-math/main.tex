...
variable \(y\) with a link function \(g\) and a linear combination of \(K-1\)
feature variables \(x_k\) (for \(K\) parameters including the intercept term).
...
\ldots, x_{K-1}]\) and \(\bbeta^\intercal \equiv [\beta_0, \beta_1, \ldots
\beta_{K-1}]\) the above equation is written as the following.
...
\begin{equation}
    ...
    1      & x_1^{(1)} & \ldots & x_{K-1}^{(1)} \\
    1      & x_1^{(2)} & \ldots & x_{K-1}^{(2)} \\
		\vdots & \vdots    & \ddots & \vdots    \\
    1      & x_1^{(N)} & \ldots & x_{K-1}^{(N)}
	\end{pmatrix}
\end{equation}
...
information and expectations (TODO: find decent reference for this if not
explain it).
...
sometimes absorb one of the parameters of a multi-parameter distribution, such as
...
\[ g(\textrm{E}[y^{(i)}]) = \rho^{(i)} + \mathbf{x}^{(i)\intercal} \bbeta \]
This adjusts the IRLS step to the following.
\begin{equation}
\left( \mathbf{X}^\intercal \mathbf{W} \mathbf{S} \mathbf{X} \right)
\bbeta_{l+1} = \mathbf{X}^\intercal \mathbf{W} \left[ \mathbf{y} -
g^{-1}\left(\boldsymbol{\rho} + \mathbf{X}\bbeta_l\right) +
...
because the likelihood is non-differentiable when \(\beta_k = 0\) for any \(k
\geq 1\).
...
so that \(D = \Sigma_i w_i D_i\) where in these expressions $D_i$ is the
unweighted component of the deviance for observation \(i\).
...
Note that at the fit minimum, this satisfies
\(\mathbf{P}^2 = \mathbf{P}\) exactly so it is a projection
matrix\footnote{However, regularization seems to break this property as
\(\mathbf{P}^2 = \mathbf{P} + \mathcal{O}(\lambda)\).} and is orthogonal to the
response residuals \(\mathbf{P}\cdot\left(\mathbf{y} - \hat{\mathbf{y}}\right)
= 0\) as can be observed from the IRLS step equation and the convergence of the
parameters such that \(\Delta \bbeta = 0\).
...
With variance weights, the denominator has a more complicated form to correct.
...
The Cook's distance is another measurement of how much the fit is impacted by
each observation. For GLM's, it's defined as
\begin{align}
  C_i &= \frac{\left(\boldsymbol{\beta} - \boldsymbol{\beta}^{(-i)}\right)^\intercal \mathrm{Cov}[\boldsymbol{\beta}]^{-1} \left( \boldsymbol{\beta} - \boldsymbol{\beta}^{(-i)}\right)}{K \phi} \\
      &= \frac{\left(\boldsymbol{\beta} - \boldsymbol{\beta}^{(-i)}\right)^\intercal \left(\mathbf{X}^\intercal \mathbf{W} \mathbf{S} \mathbf{X}\right) \left( \boldsymbol{\beta} - \boldsymbol{\beta}^{(-i)}\right)}{K \phi} \\
      &= \frac{h_i}{(1-h_i)^2} \frac{r^{(i)2}}{K\phi}
\end{align}
where \(r^{(i)} = \sqrt{\frac{w_i}{S_{ii}}}e^{(i)}\) are the Pearson residuals.

\section{Parameter scaling}

Often one wants to standardize the training data so that the features all have
zero mean and unit variance. This separates the contribution from the mean
values and puts all coefficients on the same footing. In general with weights
$w_i$ we have:

\begin{align}
  x'_k &= \frac{x_k - \mu_k}{\sigma_k} \\
  \mu_k &= \frac{\sum_k w_k x_k}{\sum_k w_k} \\
  \sigma_k &= \sqrt{\frac{\sum_k w_k (x_k - \mu_k)^2}{\sum_k w_k - \frac{\sum_k w_k^2}{\sum_k w_k}}}
\end{align}
and we can take by convention $\sigma_i = 1$ for zero-variance data columns. We
must have \(\beta'_0 + \sum_k \beta'_k x'_k = \beta_0 + \sum_k \beta_k x_k\), so
\begin{align}
  \beta_0 &= \beta'_0 - \sum_{k=1} \frac{\beta'_k \mu_k}{\sigma_k} \\
  \beta_k &= \frac{\beta'_k}{\sigma_k} \\
  \beta'_0 &= \beta_0 + \sum_{k=1} \beta_k \mu_k \\
  \beta'_k &= \beta'_k \sigma_k \\
\end{align}

The Jacobian of the transformation $\beta \rightarrow \beta'$ is given by the matrix
\begin{equation}
  \boldsymbol{\mathcal{J}}_{ij} = \frac{\partial \beta'_i}{\partial \beta_j} = \begin{pmatrix}
    1      & \mu_1 & \mu_2 & \ldots & \mu_{K-2} & \mu_{K-1} \\
    0      & \sigma_1 & 0 & \ldots & 0 & 0\\
    0      & 0 & \sigma_2 & \ldots & 0 & 0 \\
    \vdots  & \vdots & \vdots   & \ddots & \vdots & \vdots   \\
    0      & 0 & 0 & \ldots & \sigma_{K-2} & 0 \\
    0      & 0 & 0 & \ldots & 0 & \sigma_{K-1} \end{pmatrix}
\end{equation}

which means the Fisher information matrix is
\begin{equation}
  \boldsymbol{\mathcal{I}} = \boldsymbol{\mathcal{J}}^\intercal \boldsymbol{\mathcal{I}}'\boldsymbol{\mathcal{J}}
\end{equation}
where \(\boldsymbol{\mathcal{I}}'\) is the observed Fisher information matrix
(the negative Hessian at the fit minimum) in terms of the standardized
parameters $\beta'$, more easily extracted from the fit result.

Note that in a no-intercept context, data and parameters can still be scaled by
the standard deviation, but they cannot be also shifted and maintain this
property. Variable scaling can still be appled, but $\mu_k$ are effectively set
to zero (after using them to compute $\sigma_k$, that is).

\section{Covariance of coefficient estimates}

Without regularization, the covariance of the $\boldsymbol{\beta}$ is
straighforwardly the observed Fisher information matrix, which is the Hessian
at the fit minimum.
\[\textrm{Cov}[\beta_i , \beta_j] = \mathcal{I}_{ij} = -H_{ij} \]

It gets tricker when including regularization terms. If the regularization
penalty was taken as implying a true prior distribution for
$\boldsymbol{\beta}$, using the penalized Hessian (e.g. \(\mathbf{H} =
-\mathbf{X}^\intercal \mathbf{WSX} - \lambda_2 \mathbf{I} \)) might still be
justified, but this is rarely the case in practical applications.

We'll proceed to outline the argument for a more robust estimator, but not with
full rigor. For reference, see
\href{https://repository.lib.ncsu.edu/server/api/core/bitstreams/40ad2126-511c-49e8-b15f-a02683725978/content}{\emph{The
calculus of M-estimation} by Stefanski and Boos (2001)}.

Consider an estimate $\hat{\boldsymbol{\beta}}$ around a "true" value
$\boldsymbol{\beta}_0$. Asymtotically ($n \rightarrow \infty$) we should have
\(\hat{\boldsymbol \beta} \rightarrow \boldsymbol \beta_0\). The score function
\(\mathbf{J} = \nabla_\beta l\) is zero at the optimized fit. Taylor expand
 
\begin{align}
  0 &= \mathbf{J}(\boldsymbol\beta_0) + \mathbf{H}(\boldsymbol\beta_0)
  (\hat{\boldsymbol\beta} - \boldsymbol\beta_0) +
  \mathcal{O}\left((\hat{\boldsymbol\beta} - \boldsymbol\beta_0)^2 \right) \\
  \hat{\boldsymbol\beta} - \boldsymbol\beta_0 &\sim -\mathbf{H}^{-1}(\boldsymbol\beta_0) \mathbf{J}(\boldsymbol\beta_0)
  % \label{eq:}
\end{align}
and note that part of the rigor we are neglecting is showing that the
higher-order terms can be dropped.

Since $\mathbf{H}$ has no explicit dependence on $y_i$, it can essentially be
represented by its observed value (under the asymptotic assumption). The score
vector \(\mathbf{J}\) has an expectation value of zero. By the central limit
theorem, it is asymptotically normal, so we must only derive its covariance.
Take $\mathbf{J}_0$ as the unregularized score, so \(\mathbf{J} = \mathbf{J}_0
- \nabla \Lambda(\boldsymbol\beta_0)\), which is useful because the
regularization component is not random, so \( \textrm{Cov}[\mathbf{J}] =
\textrm{Cov}[\mathbf{J_0}]\). But the covariance of $\mathbf{J}_0$ is given by
the covariance of $y$, up to the data and weight matrix factors. Thus 
\(\textrm{Cov}[\mathbf{J}_0] = \mathbf{X^\intercal WSX} = -\mathbf{H}_0\),
where the other weight matrix factor probably goes into the weighted average
over the $(y-g^{-1})^2$ terms.

Now using Slutsky's theorem, since the regularized $\mathbf{H}$ act like a
constant scale, we can say asymptotically that \(\hat{\boldsymbol\beta} -
\boldsymbol\beta_0\) is normally-distributed with mean zero and covariance
\(\mathbf{H}^{-1} \mathbf{H}_0 \mathbf{H}^{-1}\).
...
-\frac{y^2}{2\phi} - \frac{1}{2}\log(2\pi\phi) \).\footnote{
...
\subsubsection{Variable variance}
...
1\). The reverse transformation is \(\mu = -\eta_1/2\eta_2\) and \(\sigma^2 =
-1/2\eta_2\). Up to a constant the log-partition function is \(A(\eta_1,
\eta_2) = -\frac{\eta_1^2}{4\eta_2} - \frac{1}{2}\log (-\eta_2) \).
This is likely a good application for a non-canonical link function for
\(\eta_2\), for instance \(\eta_2 = - \exp(\mathbf{x}\cdot\bbeta_2)\), since
\(\sigma^2 > 0\) so \(\eta_2 < 0\). Here the variance function \(V(\mu)\) could
possibly be replaced by a covariance function of the two-component expected
values of \(\mu\) and \(\sigma^2\), but checking this should require some more
careful analysis. 

The log-likelihood and its gradients in terms of the natural parameters is
\begin{align}
  \ell = \log f(y; \eta_1, \eta_2) &= \eta_2 y^2 + \eta_1 y + \frac{\eta_1^2}{4\eta_2} + \frac{1}{2} \log (-2\eta_2) + B \\
  \frac{\partial \ell}{\partial \eta_1} &= y + \frac{\eta_1}{2\eta_2} \\
  \frac{\partial \ell}{\partial \eta_2} &= y^2 - \frac{\eta_1^2}{4\eta_2^2} + \frac{1}{2\eta_2}
  % \label{eq:}
\end{align}
so there is already some mixing at this level regardless of the link functions
chosen. The 2nd derivatives won't reduce in complexity.

I wonder if this natural parameter approach is leading us astray. Maybe it's
better to consider the mean \(\mu\) and a reparameterization of the variance
\(\sigma^2 = e^{-\nu}\). This approach would generalize nicely because it just
slots into the dispersion parameter for any family. Try \(\mu =
\mathbf{x}\cdot\boldsymbol\beta\) and \(\nu =
\mathbf{x}\cdot\boldsymbol\gamma\).

\begin{align}
  \ell &= \sum_i -\frac{1}{2} (y^{(i)} - \mathbf{x}^{(i)}\cdot\boldsymbol\beta)^2 e^{\mathbf{x}^{(i)}\cdot\boldsymbol\gamma} + \frac{\mathbf{x}^{(i)}\cdot\boldsymbol\gamma}{2} \\
  \mathbf{J}_\beta &= \sum_i \mathbf{x}^{(i)}(y^{(i)} - \mathbf{x}^{(i)}\cdot\boldsymbol\beta) e^{\mathbf{x}^{(i)}\cdot\boldsymbol\gamma} \\
  \mathbf{J}_\gamma &= \sum_i -\frac{1}{2}\mathbf{x}^{(i)}\left[ (y^{(i)} - \mathbf{x}^{(i)}\cdot\boldsymbol\beta)^2 e^{\mathbf{x}^{(i)}\cdot\boldsymbol\gamma} - 1 \right] \\
  \mathbf{H}_{\beta\beta} &= \sum_i -\mathbf{x}^{(i)}\mathbf{x}^{(i)\intercal} e^{\mathbf{x}^{(i)}\cdot\boldsymbol\gamma} \\
  \mathbf{H}_{\beta\gamma} &= \sum_i \mathbf{x}^{(i)}\mathbf{x}^{(i)\intercal}(y^{(i)} - \mathbf{x}^{(i)}\cdot\boldsymbol\beta) e^{\mathbf{x}^{(i)}\cdot\boldsymbol\gamma} \\
  \mathbf{H}_{\gamma\gamma} &= \sum_i -\frac{1}{2}\mathbf{x}^{(i)}\mathbf{x}^{(i)\intercal}  (y^{(i)} - \mathbf{x}^{(i)}\cdot\boldsymbol\beta)^2 e^{\mathbf{x}^{(i)}\cdot\boldsymbol\gamma} \\
  % \label{eq:}
\end{align}
An IRLS step could be defined in block-matrix form

\begin{equation}
  \begin{pmatrix} 
    \mathbf{H}_{\beta\beta} & \mathbf{H}_{\beta\gamma} \\ 
    \mathbf{H}_{\beta\gamma} & \mathbf{H}_{\gamma\gamma} \\ 
...
Note that \(\mathbf{H}_{\beta\gamma} = \mathbf{J}_\beta \mathbf{x}^\intercal
\), which vanishes at the fit optimum. This fortunately means that
the \(\boldsymbol\beta\) and \(\boldsymbol\gamma\) results are independent (at
least once the fit converges). This suggests that an alternating algorithm that
stepped \(\beta\) and \(\gamma\) separately (ignoring
\(\mathbf{H}_{\beta\gamma}\)) would also converge. It appears that this would
be true for other families as well, at least with the exponential
parameterization of the dispersion.
...
The exponential response distribution has a simple form but the constraint on
the distribution's parameter motivates a non-canonical link function. The PDF
for an exponentially-distributed positive real value \(y\) is
...
where \(y \geq 0\) (and an implied \(-\infty\) for \(y < 0\)). The natural
parameter is \(\eta = -\lambda\) and the log-partition function is \(A(\eta) =
- \log(-\eta)\).
...
\(V(\mu) = \mu^2\), is the most straightforward and directly generalizes the
exponential GLM by maintaining its variance function.
...
= - r \log(1 - e^\eta)\), the mean is \(\mu = \frac{r(1-p)}{p}\), the canonical
link function is \(g(\mu) = \log \frac{\mu}{r+\mu}\).
...
\begin{align}
f(y; \mu, \lambda) &= \sqrt{\frac{\lambda}{2\pi y^3}}
...
&= \frac{-y/\mu^2 - 1/y + 1/\mu}{2/\lambda} + \frac{1}{2}\log\lambda + B(y)
\end{align}
...
To put in dispersed form, note that we can take \(\phi = 2/\lambda\) and then
\(\eta = -1/\mu^2\). So this is another situation where the canonical link
function doesn't disallow impossible values.
